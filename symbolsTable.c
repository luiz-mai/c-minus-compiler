...
int addOrLookSymbol(SymbolsTable *st, char *symbol, int line, int scope,
                    int arity) {

  if (!symbolExists(st, symbol) || getSymbolScope(st, symbol) != scope) {
    ...
    return getSymbolIndex(st, symbol, scope);
  }
}
...
int getSymbolIndex(SymbolsTable *st, char *symbol, int scope) {
  SymbolNode *current = st->head;
  int i = 0;
  while (current != NULL) {
    if (scope == -1) {
      ...
      if (strcmp(current->symbol, symbol) == 0 && current->scope == scope) {
        return i;
      }
    }
    current = current->next;
...
char *getSymbolName(SymbolsTable *st, int index) {
  int i = 0;
  SymbolNode *current = st->head;
  while (i < index) {
    current = current->next;
...
void printVariablesTable(SymbolsTable *st) {
  ...
  int i = 0;
  while (current != NULL) {
    printf("\nEntry %d -- name: %s, line: %d, scope: %d", i, current->symbol,
           current->line, current->scope);
    current = current->next;
    i++;
  }
  printf("\n\n");
...
void printFunctionsTable(SymbolsTable *st) {
  ...
  int i = 0;
  while (current != NULL) {
    printf("\nEntry %d -- name: %s, line: %d, arity: %d", i, current->symbol,
           current->line, current->arity);
    current = current->next;
    i++;
  }
  printf("\n");
